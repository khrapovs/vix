...
for d in np.unique(df['date']):
    ...
    dfc = ps.merge(dfc, dfy, left_on = ['date', 'ydays'], \
        right_on = ['date', 'days'])
    ...
    for g in (group1, group2):
        ...
        for cp in (left, right):
            ...
            try:
                ...
                g['dK'][cp & g['low'].isnull()] = \
                    g['high'][cp & g['low'].isnull()] \
                    - g['strike_price'][cp & g['low'].isnull()]
                g['dK'][cp & g['high'].isnull()] = \
                    g['strike_price'][cp & g['high'].isnull()] \
                    - g['low'][cp & g['high'].isnull()]
            except:
                g['sigma2'] = np.nan
                continue
